...
- `packages/react-select-async-paginate` — wrapper around `react-select` adding pagination-on-scroll support. This is the core package; the other package depends on it.
- `packages/react-select-fetch` — wrapper around `react-select-async-paginate` that loads options from a given URL.
...
- `withAsyncPaginate` (`src/withAsyncPaginate.tsx`) — HOC that wires `useAsyncPaginate`'s output into any `react-select`-compatible component. `AsyncPaginate = withAsyncPaginate(Select)` in `src/index.ts` is the main exported component.
...
Adapts `react-select-async-paginate` to a URL-based data source instead of a custom `loadOptions` function:
